...
public class WithdrawServlet extends HttpServlet{
	...
	public void updateStudent(int id, int amount,int trans_id){try{
		...
		stmt.executeUpdate("update bankaccount set type=" + "withdraw" + " where cust_id=" + id);
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		if(Globals.wdscreen==true)
		{
			setup();
			Globals.trans_id++;
			String umoney= req.getParameter("Amount");
			Globals.amount= Integer.parseInt(umoney);
			...
			rd.forward(req, resp);
			Globals.wdscreen=false;
			
		}
		else if(Globals.wdscreen==false){
		rd=req.getRequestDispatcher("/withdraw.jsp");
		rd.forward(req, resp);
		Globals.wdscreen=true;
		}
}
}